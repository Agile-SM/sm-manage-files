...
class FILE {
                ...
                if (error) {
                    ...
                    if (data.files) {
                        for(let i =0; i < data.files.length; i++) {
                            if (data.files[i].mimetype.split('video').length > 1 || data.files[i].mimetype.split('audio').length > 1 || data.files[i].mimetype.split('image').length > 1) {
                                permission = true;
                            } else {
                                permission = false;
                            }
                        ...
                        if (data.file.mimetype.split('video').length > 1 || data.file.mimetype.split('audio').length > 1 || data.file.mimetype.split('image').length > 1) {
                            permission = true;
                        } else {
                            permission = false;
                        }
    ...
    uploadFromTmp (file, bucket) {
        ...
        console.log("UPLOADFROMTMP")
        console.log("UPLOADFROMTMP")
        console.log("UPLOADFROMTMPd")
        console.log("FILEEEEE", file)
        console.log("UPLOADFROMTMP", bucket)
        console.log("UPLOADFROMTMP")
        console.log("UPLOADFROMTMP")
        console.log("UPLOADFROMTMP")
        console.log("UPLOADFROMTMP")
    ...
    uploadWithParams (params) {
        return this.COS.uploadFileParams(params);
    }
